...
boxes = [
    (0.5, 4.0, 2.4, 1.2, "L1\nMetadata Harvest", "#FFD9B3"),
    (3.5, 4.0, 2.4, 1.2, "L2\nNeo4j Knowledge Graph", "#CCE5FF"),
    (6.5, 4.0, 2.4, 1.2, "L3\nRule AST Engine", "#D5E8D4"),
    (9.5, 4.0, 2.4, 1.2, "L3.5\nGraphSAGE Fusion", "#E1D5E7"),
    (3.5, 1.0, 6.0, 1.4, "L4\nW3C-PROV + Ed25519 Evidence Pack\n100% ISO/IEC 27043 coverage", "#FCE5CD"),
]
for (x, y, w, h, t, c) in boxes:
    ax.add_patch(mpatches.FancyBboxPatch((x, y), w, h,
                  boxstyle="round,pad=0.05", fc=c, ec="black"))
    ax.text(x + w / 2, y + h / 2, t, ha="center", va="center", fontsize=9, fontweight="bold")

arrows = [(2.9, 4.6, 3.5, 4.6),
          (5.9, 4.6, 6.5, 4.6),
          (8.9, 4.6, 9.5, 4.6),
          (4.7, 4.0, 4.7, 2.4),
          (7.7, 4.0, 7.7, 2.4),
...
ax.set_title("TURRET OS — Four-Layer Architecture (with Joint Inference)",
             fontsize=12, fontweight="bold")
plt.tight_layout()
plt.savefig("paper/figures/fig1_architecture.png", dpi=200)
...
ax.axvline(0.005, color="red", ls=":", lw=1.2,
           label="0.5 % FPR deployment operating point")
ax.text(0.015, 0.62, "F1 = 0.8774", color="red", fontsize=10, fontweight="bold")

ax.set_xlabel("False Positive Rate")
ax.set_ylabel("True Positive Rate")
ax.set_title("ROC Curve — TURRET OS on D3 TSEC (60-30-10 chronological)")
...
ax.set_title("Confusion Matrix — D3 TSEC @ 0.5 % FPR\nF1 = 0.8774, MCC = 0.9770", fontsize=10)
plt.tight_layout()
...
ax.set_title("Pareto Frontier — Layer Ablations on D5 OOD_LAB_EXFILTRATOR")
...
ax.set_title("OCC-CP Robustness — Multi-Layer Architectural Rescue")
...
ax.set_title("Top-8 SHAP Feature Importance on D5 Chronological Split\n"
             "All bar labels belong to operational whitelist — zero leakage", fontsize=9.5)
for b in bars:
...
ax.set_title("Wall-Clock per 1 K Activity Records (Total = 15.0 s, σ = 1.5 s)")
...
ax.set_yscale("log")
ax.set_title("Storage Profile — D5 Synthetic-Tenant Pilot (35 u × 90 d × 2,431 records)", fontsize=9.5)
for i, v in enumerate([9.4, 4800, 1.2]):
    ax.text(i, v * 1.2, f"{v:g} MB", ha="center", va="bottom", fontsize=9.5, fontweight="bold")
plt.tight_layout()
...
ax.set_xticklabels(attrs, fontsize=8.5)
ax.set_yticks([0.5, 1.0]); ax.set_ylim(0, 1.15)
ax.set_title("ISO/IEC 27043 Attribute Coverage — 100 % (all 8 attributes)", fontsize=10, pad=15)
plt.tight_layout()
...
ax.set_title("Adversarial Robustness — Single vs Multi-Layer")
...
for i, v in enumerate(signals):
    ax.text(i, v + 0.2, str(v), ha="center", fontsize=9.5, fontweight="bold")
ax.set_ylabel("Distinct provenance signal fields extracted")
ax.set_title("Layer-1 Cross-Format Metadata Coverage", fontsize=10)
...
for b, h in zip(bars, hits):
    ax.text(b.get_x() + b.get_width() / 2, h + 2, str(h), ha="center", fontsize=9.5, fontweight="bold")
ax.set_ylabel("Number of fired rules across D5 alerts")
ax.set_title("Rule-Family Activation Profile across 583 detected alerts", fontsize=10)
ax.grid(alpha=0.3, axis="y")
plt.tight_layout()
...
print("Generated 12 figures successfully in paper/figures/")
